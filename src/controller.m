...
function c = controller(name, Kd, Kp, Ki)
    ...
    if strcmpi(name, 'pd')
        c = @(state, thetadot, desired_state) pd_controller(state, thetadot, desired_state, Kd, Kp);
    elseif strcmpi(name, 'pid')
        c = @(state, thetadot, desired_state) pid_controller(state, thetadot, desired_state, Kd, Kp, Ki);
    else
